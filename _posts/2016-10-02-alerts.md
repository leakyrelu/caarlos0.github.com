...
We also have an alert that is fire if there are any queued jobs on Travis.
...
ContaAzul issues these kind of invoices, so, we scripts that query our
database and alert us if the queue is growing too fast. This way, we can let
...
1. you're probably will alert the symptom anyway;
